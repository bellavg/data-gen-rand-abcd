...
\todo{make more concise also to evaluate what to put in the appendix and what to keep here write that as a permanent note so my thesis supervisor can see so not a ocmment}
...
Nothing in this subsection is a contribution of this thesis. The departures below are
GNN+'s \citep{luo2024gnnplus}, each cited to the equation of that paper it comes from;
they are written out because the reduction claims
...
configuration. The settings specific to this work are collected at the end of the
subsection. The classic GCN layer \citep{kipf2017gcn} that GNN+ modifies is
\begin{equation}
...
\end{equation}
Placing $\sigma$ on each message rather than on the aggregate follows the GNN+ reference
implementation; its Eq.~6 writes the activation outside the sum. The per-edge coefficient $c_{uv}$ is the one place~\eqref{eq:method:message} is
...
per edge when graphs are cached rather than at every forward pass; disabled, it is $1$
and no edge-weight tensor is carried at all. It is disabled here. The same
edge-weight channel is what the exact-compression track
of~\ref{sec:method:architecture:exact} reuses: setting $c_{uv}$ to the edge multiplicity
$w_{uv}$ scales the message \emph{outside} $\sigma$, which is what makes one merged
message equal the sum of the messages it replaces.
...
\begin{align}
    ...
    \label{eq:method:ffn}
\end{align}
with $W_1^{l} \colon 128 \rightarrow 256$ and $W_2^{l} \colon 256 \rightarrow 128$. The
published layer carries one normalisation that~\eqref{eq:method:residual} does not. Its
Eqs.~7--9 normalise the aggregated message \emph{before} the activation and before the
residual is added, and its Eq.~10 closes the FFN by normalising that block's own residual
sum; the GNN+ reference implementation keeps both and inserts a third ahead of the FFN,
the bracketing arrangement of GraphGPS \citep{rampasek2022graphgps}. Only the message
normalisation is dropped here. The two dropouts of~\eqref{eq:method:ffn} likewise follow
the reference implementation, which Eq.~10 does not show.
...
$W_{\text{PE}} \colon 1 \rightarrow 32$, so $h_v^{0} \in \mathbb{R}^{160}$. GNN+ holds its
concatenation at the model width, projecting the joined vector back down in Eq.~12 and
narrowing the node projection beforehand in its implementation. Keeping all 160
dimensions here means the
first layer maps $160 \rightarrow 128$ and is therefore the one layer whose residual
...
\begin{itemize}
    ...
    where GCN+ carries the $1 / \sqrt{\hat{d}_u \hat{d}_v}$ of~\eqref{eq:method:gcn} into
    its own layer. Fan-in in an AIG is fixed by node type, so the coefficient, taken over
    fan-in and without the self-loop, is constant within each type pair and is zero on
    every edge leaving a primary input. GNN+ supplies no evidence either way: its
    ablations \citep[Tables~5--6]{luo2024gnnplus} cover the six techniques it adds and not
    the GCN coefficient it inherits, and its own edge-aware layer disables both that
    coefficient and the self-loop, so this setting agrees with the reference
    implementation where the implementation departs from Eq.~6. The normalised variant is
    implemented and reachable through the model
    configuration, not exposed as a training flag.
...
\todo{go over this when comparins to exact sumamrizaiotn related work}