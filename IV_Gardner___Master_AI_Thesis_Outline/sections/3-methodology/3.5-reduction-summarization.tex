...
A coarsening is a quotient. A merge map $\mu \colon \mathcal{V} \rightarrow \mathcal{V}'$
...
Four candidates span domain-specific, exact and general-purpose, with a random control that
...
One rewrite turns any merge map into a graph, so that methods differ in what they merge and
...
\end{equation}
The PI/PO interface is the circuit's contract with the rest of the design, and dissolving it
...
Two AIG-native candidates, with the choice between them open because neither one's
compression on this corpus is known.
...
\emph{Cone coarsening} merges along two axes. Chains of single-fanout gates contract into one
...
The exact anchor of the study, and the one candidate whose merge map is defined by what the
...
Structural hashing has already removed the one-hop equivalences
...
Uniformly random nodes of the same type are merged until the node count reaches a target. The
...
As a matching instrument, it is what makes RQ4 answerable for this family at all. Compression
