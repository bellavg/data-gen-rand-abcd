...
of~\ref{sec:prelim:gnn:expressivity}.
%TODO decide the count-cap arm. Measured on seed designs the cap is inert on AIGs, since
%fan-in is fixed by node type, so $c = 1$ and $c = \infty$ give the same partition and the
%planned two-point sweep would measure it twice. Re-measure on the corpus before writing this.
The depth is set to $d = L$, the encoder's message-passing depth, and that equality is the
justification, not a convenience. Refinement grows finer with depth, so the two failure
directions are not symmetric: a larger $d$ separates nodes the encoder cannot tell apart, which
stays exact but forfeits compression, whereas a smaller $d$ merges nodes the encoder \emph{can}
separate, and the losslessness argument fails. Exactness requires $L \leq d$, and $d = L$ is
therefore the coarsest, most compressing depth that keeps the guarantee.

% Superseded, the two clauses were the wrong way round: a smaller $d$ merges less and gains
% nothing, while a larger one merges nodes the encoder can separate, and the losslessness
% argument fails.
% Also cut, pending the count-cap decision above: Only its two named endpoints, $c = \infty$
% and $c = 1$, are run, since intermediate caps are bracketed by them and each value costs a
% training run.
