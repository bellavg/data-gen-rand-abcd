...
\todo{evaluate which of these i will actulaly do shoudl be the base scores the means and all that for comparatability and compared to random and CIs}
\todo{is ranodmly intialized GNN a baseline? i think so but i need to check the literature}
...
\todo{i will provavly not report both just the proper one the correct one that was originally used in paper
}
...
of the score transfers (\ref{sec:results:rq1:baselinemodels}). \todo{do  i need this?}

\todo{this shoudl really be mostly on how i adapted my baselines ot the task.
 so recipe was not inputted i cut off that branch}