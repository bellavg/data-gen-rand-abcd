...
Each family below is fixed by the quantity its operator controls. Methods within a family
differ only in how that quantity is chosen.
% Original: Each family below is fixed by the quantity its operator controls, and the methods inside a
...
whose endpoints land in different blocks, so a partitioner controls only the cut. The
reference objective is the balanced minimum cut,
\begin{equation}
...
\end{equation}
with imbalance tolerance $\varepsilon$. METIS is the standard approximation to this
NP-hard problem \citep{karypis1997metis}. Every cut edge in~\eqref{eq:relwork:mincut}
counts one regardless of what it connects, the only term the domain-aware variant
of~\ref{sec:method:partition:spanmetis} changes.

Two references, not competitors, complete the picture. Hash partitioning draws $\pi(v)$
uniformly and independently of $\mathcal{E}$, the default where partition cost matters
more than quality \citep{zheng2020distdgl}, so a structure-aware objective earns its cost
only by beating each edge's $1/k$ survival probability. Ordering-based slicing sorts
$\mathcal{V}$ by a scalar the graph already carries into $k$ equal blocks
(\ref{sec:method:partition:levelslice}).
% Original: A $k$-way partition $\pi \colon \mathcal{V} \rightarrow \{1, \dots, k\}$ deletes every edge
% whose endpoints land in different blocks, so a partitioner controls the cut and nothing
% else. The reference objective is the balanced minimum cut, with imbalance tolerance
% $\varepsilon$. The problem is NP-hard, and METIS is the standard
...
on $\mathcal{V}' \subseteq \mathcal{V}$, differing in what they promise the survivor
preserves. The strongest promise available at near-linear cost bounds stretch: a
$t$-spanner is a subgraph $H \subseteq G$ with
\begin{equation}
...
trade-off in linear time \citep{baswana2007spanners}. The guarantee is paid for out of
dense short-range redundancy: a graph that lacks it returns a spanner nearly as large as
itself, the negative result in~\ref{sec:method:sparse:forest}.
...
spanning forest keeps $|\mathcal{V}| - c$ edges on $c$ components, a connectivity backbone
at a compression fixed by the graph, not a setting \citep{kruskal1956mst}. Independent
edge removal, a per-epoch GNN regulariser \citep{rong2020dropedge}, keeps each edge with
probability $1 - p$. Drawn once, it becomes a freely tunable matched-compression control.
Centrality pruning keeps the $\lceil \kappa |\mathcal{V}| \rceil$ highest-scoring nodes
under PageRank \citep{brin1998anatomy,langville2003pagerank} as an induced subgraph, so
every edge incident on a discarded node goes with it. The two ratios
of~\eqref{eq:prelim:retention} separate here: the first two hold $\eta_{\mathcal{V}} = 1$,
the last moves both.
% Original: Sparsification keeps a subset $\mathcal{E}' \subseteq \mathcal{E}$, or an induced subgraph
...
% a bound on stretch. The guarantee is paid for out of dense
...
of~\ref{sec:prelim:reduction:summarization}, and strands differ in what counts as grounds
to merge.
% Original: Coarsening merges nodes into super-nodes under the merge map
...
\citep{grohe2014colourrefinement}. \citet{bollen2023exact} quotient a graph by its $d$-step
classes and show a depth-$d$, multiset-aggregator network trained on the quotient
reproduces the original's output exactly if the graph is a multigraph with super-edge
multiplicities and per-class-constant features (\ref{sec:prelim:reduction} formalizes
this); a cap $c$ truncates the multiset, with $c = 1$ giving bisimulation, coarser and no
longer exact for a counting model. Both hold here by construction, so refinement at
$c = \infty$ is lossless
for this encoder and serves as the study's positive control rather than one more
contender (\ref{sec:method:summary:schema}, \ref{sec:method:architecture:exact}). k-SNAP
\citep{tian2008ksnap}, the depth-one case, groups by attribute and neighbour class and is
cited but not run.
% Original: The exact strand merges only nodes that are provably indistinguishable. Colour
% refinement~\eqref{eq:prelim:cr} computes the coarsest equitable partition
...
\citep{loukas2019localvariation}, surveyed by \citet{chen2022coarsening}, and Kron
reduction has a directed extension \citep{sugiyama2023kron}. Guarantees stated for a
symmetric Laplacian's spectrum do not survive the move to a directed graph unexamined, so
the strand is surveyed here, not run.
% Original: The spectral and classical strand descends from scientific computing. Local Variation
% coarsening contracts node sets under a restricted spectral approximation bound
...
Convolution matching, the GNN-aware strand \citep{dickens2024convmatch}, merges nodes the
convolution itself cannot separate, rather than some graph property, at about 95\% of
full-graph performance for 1\% of the node count on node classification, the strongest
general-purpose criterion published and the bar a domain-specific method must clear
(\ref{sec:method:summary:convmatch}). Universal Graph Coarsening merges nodes colliding
under locality-sensitive hashing in linear time \citep{ugc2024}, the cheap end of the same
axis. \citet{shabani2023summarization} survey both strands with GNNs.
% Original: Convolution matching is the GNN-aware strand \citep{dickens2024convmatch}: nodes merge when
...
Compression is a free parameter for only some coarseners, so no procedure makes two real
methods meet at one ratio, while a random merge reaches any ratio exactly. Each method is
therefore compared against a random control at its own achieved compression
(\ref{sec:method:summary:random}).
% Original: Uniform merging to a stated target is the control the rest are read against. Compression is
...
and its descendants synthesise a new small graph by gradient matching instead of merging
the real one \citep{jin2022gcond}, so (i) no correspondence exists between synthetic and
...
cross-state inference on.
% Original: Condensation is the family this thesis excludes (\ref{sec:intro:scope:reductions}). GCond
...
Two systems already train on a reduced graph and serve the original. SCAL coarsens the
training graph and applies the same weights to the full graph at inference
\citep{huang2021scal}, the shared-weights mechanism RQ5 tests. \citet{generale2022scaling}
instead transfer weights through the node-to-super-node mapping for a node-level task on
knowledge graphs, finding by ablation that a super-node's \emph{content} is the deciding
factor, why member counts and level statistics travel with super-nodes here
(\ref{sec:method:summary:mergemap}). Node-level prediction needs a mapping back to
individual nodes. Graph-level regression needs only an input space both structural states
inhabit (\ref{sec:method:summary:schema}), so shared weights suffice here.
% Original: Two systems already train on a reduced graph and then serve the original. SCAL coarsens the
% training graph and applies the same weights to the full graph at inference
