...
\begin{equation}
    w_{u'v'} = \bigl| \{\, (u, v) \in \mathcal{E} :
        \mu(u) = u',\ \mu(v) = v' \,\} \bigr|,
    \qquad m_{v'} = \bigl| \mu^{-1}(v') \bigr|,
    \label{eq:prelim:multiplicity}
\end{equation}
writing $m_{v'}$ for the number of members a cluster holds,
...
\begin{equation}
    ...
    \;=\; \sum_{u' \in \mathcal{N}'(\mu(v))}
        \frac{w_{u' \mu(v)}}{m_{\mu(v)}} \,
        \psi \bigl( h_{u'}, e_{u' \mu(v)} \bigr) ,
    \label{eq:prelim:exact-aggregation}
\end{equation}
with $\mathcal{N}'$ the quotient neighbourhood. The coefficient is the multiplicity
\emph{per member of the target cluster}, not the multiplicity itself: the left side aggregates
at one node $v$, whereas $w_{u'v'}$ counts member edges across all $m_{v'}$ members of $v'$.
The two coincide only at $m_{v'} = 1$, that is only where nothing merged, so carrying
$w_{u'v'}$ alone would overstate every message by the size of the receiving cluster.

Uniformity of the per-member count is what makes the coefficient well defined: each member of
$v'$ must have the same number of neighbours in $u'$. That does not hold for an arbitrary
partition. It holds once refinement has run to a depth at least the number of layers read
(\ref{sec:method:summary:wl}), which is why the guarantee is stated against a matched
refinement depth rather than unconditionally.

Where the multiplicity enters decides whether~\eqref{eq:prelim:exact-aggregation} holds:
$\psi$ is nonlinear, so
...
$\mathcal{V}'$ must weight each super-node by its member count $m_{v'}$
of~\eqref{eq:prelim:multiplicity} to reproduce pooling over $\mathcal{V}$:
\begin{equation}
