...
% my model against baselines SOTA
% data splitting for AIGS dont like how this is phrased this subquesoitn

\subsubsection{RQ1a: Protocol Sensitivity}
\todo{redo dont like the phrasing}
...
\todo{redo this completely and also why only have hypothesis here and not in others?}
...
\label{sec:intro:rq4}
\todo{aig-specific/ domain-knowledge informed....}
...
equivalent compression?  If domain knowledge does
...
them, and~\ref{sec:discussion:limitations:scalability} elaborates on what does and does
not extrapolate.

\subsubsection{Reduction Families Considered}
\label{sec:intro:scope:reductions}
\todo{chosen because of survery on graph reduction paper}
...
\todo{to consider something more specific on summairzation exact vs approximate? the proving of exactness bollen style?}