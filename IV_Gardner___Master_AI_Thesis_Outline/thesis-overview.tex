...
\begin{itemize}
    \item \textbf{Data Splitting:} Three grouping strategies are compared to isolate leakage sources: (1) \textit{design-disjoint} (default, 80/10/10 by design count) --- no circuit family spans more than one split; (2) \textit{lineage-disjoint} --- same-circuit different-optimization-state samples kept together, but distinct lineages of a design may split; (3) \textit{random} (row-level, no grouping) --- the leakiest baseline, quantifying how much naive splitting inflates reported accuracy.
...
The design/lineage/random split-methodology ablation (see Data Splitting, above) is reported here, alongside the full-AIG baseline metrics below.
