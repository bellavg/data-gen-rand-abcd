...
def collect_random_metadata(random_source, full_dataset_path, design):
    ...
    """
    design_metadata_path = os.path.join(random_source, 'bench', design, 'metadata', f'{design}.csv')
    target_metadata_path = os.path.join(full_dataset_path, 'metadata', 'stats', f'{design}.csv')
    
    if not os.path.exists(design_metadata_path):
        print(f"  Warning: Metadata file not found for {design}: {design_metadata_path}")
        return False
        ...
        if file_type == 'csv':
            ...
            if 'synthesisstatistics.pickle' in source_file:
                ...
                if isinstance(df, dict) and design in df:
                    ...
                    canonical_df = convert_to_canonical_format(design_data, design, 'openabc')
                else:
                    print(f"  Warning: Could not find {design} in synthesis statistics pickle")
                    return False
            else:
...
def convert_to_canonical_format(df, design, _source_type):
    ...
    column_mapping = {
        ...
        'gate_count': 'nodes',
        'num_gates': 'nodes',
        'BUFF': 'edges',  # OpenABC-D uses BUFF for buffer edges
        'NOT': 'edges',   # OpenABC-D uses NOT for inverter edges
        'edge_count': 'edges',
        'PI': 'num_PI',
    ...
    for col in canonical_columns:
        if col in df_mapped.columns:
            ...
            if 'recipe_id' in canonical_df.columns and 'step_id' in canonical_df.columns:
                canonical_df[col] = canonical_df.apply(
                    lambda row: f"base_aigs/{design}/{design}_syn{row['recipe_id']}_step{row['step_id']}.aig" 
                    if pd.notna(row.get('recipe_id')) and pd.notna(row.get('step_id'))
                    else f"base_aigs/{design}/{design}_orig.aig", axis=1
                )
            else:
                canonical_df[col] = f"base_aigs/{design}/{design}_orig.aig"
        elif col in ['avg_fanout', 'max_fanout']:
            # Calculate or estimate if not available
            if col == 'avg_fanout':
                canonical_df[col] = 2.0  # Default for AIG
            else:  # max_fanout
                canonical_df[col] = 2    # Default for AIG
        else:
            # Default values for missing columns
            canonical_df[col] = 0 if col in ['nodes', 'edges', 'num_PI', 'num_PO', 'depth'] else ''
...
def validate_metadata_file(metadata_path):
    ...
    try:
        ...
        if list(df.columns) != expected_columns:
            return False, f"Column mismatch. Expected: {expected_columns}, Got: {list(df.columns)}"
...
def main():
    parser = argparse.ArgumentParser(description='Collect and organize metadata for Full AIG Dataset')
    ...
    parser.add_argument('--validate', action='store_true', help='Validate generated CSV files')
    ...
    parser.add_argument('--random-source', help='Override Random AIG dataset source path')
    parser.add_argument('--openabc-source', help='Override OpenABC-D dataset source path')
    ...
    for design in designs_to_process:
        ...
        if design in RANDOM_DESIGNS and sources['random']:
            ...
            for random_source in sources['random']:
                if collect_random_metadata(random_source, args.full_dataset_path, design):
                    success_count += 1
                    break
            ...
            for openabc_source in sources['openabc']:
                if collect_openabc_metadata(openabc_source, args.full_dataset_path, design):
                    success_count += 1
                    break
