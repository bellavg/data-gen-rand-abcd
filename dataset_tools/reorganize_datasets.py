...
def _is_zip_sane(zip_path):
    """
    Perform a lightweight sanity check for zip files.
    """
...
def _needs_recopy(source_path, target_path, verify_existing=False):
    """
    Decide whether target should be copied/replaced from source.
    """
    ...
    if target_path.endswith(".zip") and (not _is_zip_sane(target_path)):
        return True

    if target_path.endswith(".aig") and target_size == 0:
        return True

    return False


def _missing_or_suspect(path, verify_existing=False):
    """
    Check if an extracted output file is missing or suspicious.
    """
...
def copy_design_files_as_is(
    source_design_path, target_design_path, verify_existing=False
):
    """
    Copy top-level files from source design folder to target folder unchanged.
    Existing extra files already present in target (e.g. previously extracted step AIGs)
    are preserved and never deleted by this function.
    """
    copied = 0
    if not os.path.isdir(source_design_path):
        return copied

    for entry_name in os.listdir(source_design_path):
        source_entry = os.path.join(source_design_path, entry_name)
        target_entry = os.path.join(target_design_path, entry_name)

        if not os.path.isfile(source_entry):
            continue

        if _needs_recopy(source_entry, target_entry, verify_existing=verify_existing):
...
def create_full_dataset_structure(output_dir):
    ...
    structure = {
        ...
        "metadata": {"stats": None, "library": None},
    }
...
def process_random_dataset(
    random_dataset_path,
    full_dataset_path,
    keep_zipped_aigs=False,
    verify_existing=False,
):
    """
    Process the Random AIG dataset and move files to FULL_DATASET structure.
    """
    mode = "zip-preserving" if keep_zipped_aigs else "extraction"
    print(f"Processing Random AIG dataset ({mode} mode)...")
    ...
    for design in RANDOM_DESIGNS:
        ...
        if keep_zipped_aigs:
            copied_files = copy_design_files_as_is(
                design_path,
                target_path,
                verify_existing=verify_existing,
            )
            processed_count += copied_files
            print(f"  Mirrored {design}: copied {copied_files} top-level files as-is")
            continue
        ...
        if os.path.exists(orig_file):
            ...
            if _needs_recopy(orig_file, target_orig, verify_existing=verify_existing):
        ...
        for recipe_id in range(1500):  # 0 to 1499
            ...
            step_file_names = [
                f"{design}_syn{recipe_id}_step{step_id}.aig" for step_id in range(1, 22)
            ]
            missing_steps = [
                aig_name
                for aig_name in step_file_names
                if _missing_or_suspect(
                    os.path.join(target_path, aig_name),
                    verify_existing=verify_existing,
                )
            ]
            ...
            if os.path.exists(zip_file):
                with zipfile.ZipFile(zip_file, "r") as zf:
                    ...
                    for aig_name in missing_steps:
                        if aig_name in zip_entries:
                            target_file = os.path.join(target_path, aig_name)
                            with (
                                zf.open(aig_name) as source,
    ...
    if os.path.exists(synscripts_src):
        for design in RANDOM_DESIGNS:
            ...
            if os.path.exists(design_scripts_src):
                ...
                design_zip = os.path.join(
                    full_dataset_path, "synScripts", f"{design}.zip"
                )
                if not os.path.exists(design_zip):
                    with zipfile.ZipFile(design_zip, "w") as zf:
                        for script_file in glob.glob(
                            os.path.join(design_scripts_src, "*.script")
                        ):
...
def process_openabc_dataset(
    openabc_dataset_path,
    full_dataset_path,
    keep_zipped_aigs=False,
    verify_existing=False,
):
    ...
    """
    mode = "zip-preserving" if keep_zipped_aigs else "extraction"
    print(f"Processing OpenABC-D dataset ({mode} mode)...")
    ...
    for design in OPENABC_DESIGNS:
        ...
        for recipe_id in range(1500):
            ...
            if os.path.exists(zip_file):
                ...
                for aig_name in missing_steps:
                    aig_file = os.path.join(design_path, aig_name)
        ...
        if os.path.exists(synscripts_src):
            ...
            if os.path.exists(synscripts_zip):
                ...
                target_zip = os.path.join(
                    full_dataset_path, "synScripts", "synScripts.zip"
                )
        ...
        if os.path.exists(source_path):
            if os.path.isdir(source_path):
                ...
                if len(lib_files) > 0:
                    ...
                    print("  Created library archive: nangate45.lib.zip")
            else:
                # Single library file
...
def create_dataset_manifest(full_dataset_path):
    ...
    manifest = {
        ...
        "version": "1.0",
        "designs": {"random": RANDOM_DESIGNS, "openabc": OPENABC_DESIGNS},
        "structure": {
            "base_aigs": "Original AIG files and/or per-recipe synthesized zip archives",
...
def main():
    parser = argparse.ArgumentParser(
        description="Reorganize datasets into FULL_DATASET structure"
    )
    parser.add_argument(
        "--random-dataset",
        required=True,
    ...
    parser.add_argument(
        "--openabc-dataset", help="Path to OpenABC-D dataset (if separate from random)"
    )
    parser.add_argument(
        "--output", "-o", required=True, help="Output directory for FULL_DATASET"
    )
    parser.add_argument(
        "--lib-paths", nargs="*", help="Paths to library files or directories"
    )
    parser.add_argument(
        "--keep-zipped-aigs",
        action="store_true",
        help="Keep synthesized syn*.zip files in base_aigs instead of extracting all AIGs; existing extracted AIGs already present in output are preserved",
    )
    parser.add_argument(
        "--verify-existing",
        action="store_true",
        help="Verify existing source-backed output files (size/zip sanity) and recopy suspect or mismatched ones; does not delete existing extra files",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not args.dry_run:
        if not process_random_dataset(
            args.random_dataset,
            args.output,
            keep_zipped_aigs=args.keep_zipped_aigs,
    ...
    if args.openabc_dataset:
        ...
        if not args.dry_run:
            if not process_openabc_dataset(
                args.openabc_dataset,
                args.output,
                keep_zipped_aigs=args.keep_zipped_aigs,
                verify_existing=args.verify_existing,
            ):
        ...
        print(
            "Note: No separate OpenABC dataset path provided. Assuming it's included in Random dataset."
        )
