...
def extract_aig_stats_from_file(aig_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            stats_match = re.search(r'nodes:\s*(\d+).*inputs:\s*(\d+).*outputs:\s*(\d+).*levels:\s*(\d+)', output, re.DOTALL)
            if stats_match:
    ...
    try:
        with open(aig_file_path, 'rb') as f:
            ...
            if first_line.startswith('aig'):
                ...
                if len(parts) >= 6:
                    _, inputs, latches, outputs, and_gates = map(int, parts[1:6])  # max_var not used
...
def collect_metadata_for_design(design: str, base_dir: str) -> int:
    ...
    processed_recipes = 0
    error_summary: Dict[str, int] = {'total_errors': 0, 'total_warnings': 0, 'recipes_with_errors': 0, 'recipes_with_warnings': 0}
    ...
    for recipe_id in range(1500):  # 1500 synthesis recipes
        ...
        try:
            ...
            if warnings:
                ...
                significant_warnings = [w for w in warnings if 'zip operation' not in w.lower()]
                if significant_warnings:
                    error_details.append(f"Recipe {recipe_id} warnings: {', '.join(significant_warnings)}")
            ...
            for step_id, (aig_file, stats) in enumerate(zip(aig_files, stats_list), 1):
                if stats:  # Only add if we have valid statistics
                    ...
                    actual_step = int(step_match.group(1)) if step_match is not None else step_id
