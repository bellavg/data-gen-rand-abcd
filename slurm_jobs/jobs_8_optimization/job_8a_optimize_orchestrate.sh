...
echo "STEP 8a: regenerating optimization scripts (algorithm=${ALGORITHM}, input_source=base_aigs, design_group=${DESIGN_GROUP})"
GEN_ARGS=(
    --base-dir "$BASE_DIR"
...
PY
    ...
    tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/opt_orch_${SLURM_JOB_ID:-local}_XXXXXX")"
...
echo "STEP 8a complete: processed=${processed_designs} skipped=${skipped_designs} total=${expected_designs} time=$(date)"
