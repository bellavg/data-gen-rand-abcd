#!/bin/bash
#SBATCH --job-name=8c_opt_syn4_fix
...
PY
    then
        ...
        echo "STEP 8c FIX: skipping design ${design_name} (dynamically verified complete: exact file count matches)"
        continue
    fi
    # ---------------------------------------------------------

    echo "STEP 8c FIX: starting design ${design_name} at $(date)"

    # WIPE the broken/corrupt outputs so the python script has a clean slate
    echo "  - Wiping broken old outputs for $design_name"
    rm -f "$FULL_DATASET/metadata/raw_logs/$design_name/$OUTPUT_TIER/$ALGORITHM/summary.json"
    rm -f "$out_zip"
    rm -rf "$FULL_DATASET/optimized_aigs/$ALGORITHM/$OUTPUT_TIER/$design_name"
    ...
    echo "STEP 8c FIX: Done ${design_name} at $(date)"
    rm -rf "$tmp_dir"
done < "$designs_file"

echo "=========================================="
echo "STEP 8c FIX COMPLETE: Processed=$processed_designs Skipped=$skipped_designs"
echo "Time: $(date)"
echo "=========================================="