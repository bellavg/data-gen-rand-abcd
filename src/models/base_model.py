...
try:
	from model_utils import get_pyg_pool, get_batch_positional_encoding
except ImportError:  # pragma: no cover - fallback for package-style imports
	from models.model_utils import get_pyg_pool, get_batch_positional_encoding

try:
	from layers.gcn import GCNEncoder
	from layers.gine import GINEEncoder
	from layers.graphgps import GraphGPSEncoder
	from layers.transformer_conv import TransformerConvEncoder
	from layers.vanilla_mpnn import MPNNEncoder
	from layers.rgcn import RGCNEncoder
	from layers.egin import GraphEGIN
...
class UnifiedGraphBaseModel(nn.Module):
	...
	def __init__(
		...
		self.head = nn.Identity() if self.encoder_name == "egin" else nn.LazyLinear(task_out_dim)

	def _derive_edge_type(self, edge_attr: Optional[torch.Tensor], edge_type: Optional[torch.Tensor]) -> torch.Tensor:
		if edge_type is not None:
			return edge_type.long()

		if edge_attr is None:
			raise ValueError("Provide edge_type or edge_attr so edge_type can be derived.")
		...
		if edge_attr.size(-1) == 1:
			_, edge_type_idx = torch.unique(edge_attr.view(-1), sorted=True, return_inverse=True)
			return edge_type_idx.long()
