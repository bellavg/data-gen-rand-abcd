"""Vendored from zyzheng17/DeepGate4-ICLR-25, src/models/dg2.py.

Source: https://github.com/zyzheng17/DeepGate4-ICLR-25/blob/master/src/models/dg2.py
...
This is DeepGate4's *tokenizer*: the DeepGate2 DAG recurrent encoder that
produces the disentangled structural (`hs`) and functional (`hf`) embeddings
the sparse transformer then refines. dg4.py instantiates it as
`self.tokenizer = DeepGate2(dim_hidden=self.hidden)`.

Unmodified apart from deleting two unused imports upstream carries here,
`import deepgate as dg` and `import copy` (neither name is referenced
anywhere in this file; the first would additionally require the external
`python-deepgate` package, which is not a dependency of this project, so
importing this module would fail outright). No behavioural change.

`generate_hs_init` is dead code in this project -- DeepGate4's own
`large_ckt` path never calls it either, initialising PI embeddings through
`reset_history` instead -- but it is left in place for fidelity to the
original file, matching how baselines/openabc_synthnet/model.py keeps
upstream's unused `SynthFlowEncoder`/`SynthConv`.

IMPORTANT for readers of `forward()`: `PI_prob` is accepted and then never
used -- `hf` is initialised to zeros regardless. That is upstream's own
behaviour, not a porting slip, and it is why this project's port does not
need DeepGate's random-simulation logic-1 probabilities (which would
otherwise require building upstream's C++ simulator). See regressor.py.
"""
