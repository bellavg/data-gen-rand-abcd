"""Vendored from NYU-MLDA/OpenABC, models/qor/SynthNetV3/model.py.

Source: https://github.com/NYU-MLDA/OpenABC/blob/master/models/qor/SynthNetV3/model.py
License: BSD 3-Clause (see LICENSE_UPSTREAM in this directory).

Kept as close to the upstream file as possible. The only change from upstream
is `allowable_features['node_type']`: OpenABC-D uses 3 node types, this
project's AIGs use 4 (constant, PI, AND gate, PO) -- see
config.NODE_INPUT_DIM. `SynthFlowEncoder`, `SynthConv`, and `SynthNet` are
kept unmodified even though this project's `regressor.py` does not use them
(there is no variable synthesis recipe to condition on here, since training
targets a single fixed algorithm) -- they are left in place for fidelity to
the original file rather than trimmed out.
"""
...
allowable_features = {
    # OpenABC-D upstream: [0,1,2] (3 node types). This project's AIGs encode
    # 4 node types (constant, PI, AND gate, PO) -- see config.NODE_INPUT_DIM.
    'node_type' : [0,1,2,3],
    'gate_type' : [0,1,2]
}
