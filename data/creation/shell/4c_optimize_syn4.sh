...
#SBATCH --array=0-54
#SBATCH --output=logs/opt_syn4/4c_opt_Syn4_%A_%a.out
...
echo "STEP 4c: start job=${SLURM_JOB_ID:-local} host=$(hostname) time=$(date)"
...
# 2. STAGE AIGS (Fixes 128 path errors)
